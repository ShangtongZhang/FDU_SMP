...
function listCommnet(objs){
	...
	//$.each(objs, function displayStudent(id,item){
		...
		cell1.innerHTML = "主题：";
		cell1.className = "colomn";
		//cell1.setAttribute("width","3em");
		newNode1.appendChild (cell1);
		
		var cell2=document.createElement ("td");
		cell2.innerHTML = item.evaluation.topic;
		newNode1.appendChild (cell2);
		...
		cell3.innerHTML = "评价：";
		//cell3.setAttribute("width","3em");
		newNode2.appendChild (cell3);
		
		var cell4=document.createElement ("td");
		cell4.innerHTML = "<div style=\"word-break:break-all\">"+item.evaluation.comment+"</div>";
		...
		cell5.innerHTML = "时间：";
		//cell3.setAttribute("width","3em");
		newNode3.appendChild (cell5);
		
		var cell6=document.createElement ("td");
		cell6.innerHTML = item.evaluation.createdTime;
		...
		cell7.innerHTML = "时长：";
		//cell3.setAttribute("width","3em");
		newNode4.appendChild (cell7);
		
		var cell8=document.createElement ("td");
		cell8.innerHTML = item.evaluation.period;
		newNode4.appendChild (cell8);
		...
		cell9.innerHTML = "标签：";
		//cell3.setAttribute("width","3em");
		newNode5.appendChild (cell9);
		
		var cell10=document.createElement ("td");
		
		var tempHtml = "";
		$.each(item.tags, function displayStudent(id,item){
			tempHtml += item.tagName + "、";
		});
		if(tempHtml.length != 0)
		    cell10.innerHTML = "<div style=\"word-break:break-all\">"+tempHtml.substring(0,tempHtml.length - 1)+"</div>";
		else
			cell10.innerHTML = tempHtml;
		...
		var cell6=document.createElement ("td");
		cell6.innerHTML = "<button type=\"button\" class=\"btn btn-default\" onClick=\"deleteComment("+item.evaluation.id+");\">删除</button>";
		newNode3.appendChild (cell6);
		...
		*/
		...
		$.each(objs, function displayStudent(id,item){
			...
			var commentHtml = "";
			$.each(item.tags, function displayStudent(id,item){
				//commentHtml += "<p style=\"word-break:break-all; height:1em; margin:0;\">"+item.tagName + "<p>";
			...
			//console.log(vis);
			if(vis == 0)
			    opeHtml = "<table><tr><td><input type=\"button\" class=\"btn btn-default btn-comment\" value=\"查看\" onClick=\"viewComment("+item.evaluation.id+");\" id=\"see"+item.evaluation.id+"\"></td>"+
                "<td><input type=\"button\" class=\"btn btn-default btn-comment\" value=\"删除\" onClick=\"deleteComment("+item.evaluation.id+");\"></td></tr></table>";
